                ...
                >
                    ...
                    <div className="flex justify-between items-center px-10 py-8 shrink-0">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-3xl font-black tracking-tighter uppercase">Domain Nexus</h2>
                                <p className="text-[10px] font-bold opacity-30 uppercase tracking-[0.4em] mt-1">Digital Surface Analysis</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="flex-1 flex flex-col max-w-7xl mx-auto w-full px-10 pb-10 gap-10 overflow-hidden mt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 shrink-0">
                            {[
                                { label: "Tracked Domains", value: aggregateStats.totalSites, icon: Network, color: "text-blue-500" },
                                { label: "Active Browsing", value: formatDuration(aggregateStats.totalTime), icon: Clock, color: "text-emerald-500" },
                                { label: "Primary Vector", value: aggregateStats.mostVisited, icon: BarChart3, color: "text-amber-500" }
                            ].map((stat, i) => (
                                <motion.div
                                    key={i}
                        ...
                        <div className="flex-1 min-h-0 flex flex-col md:flex-row gap-10">
                            ...
                            <div className="flex-1 flex flex-col gap-6 min-h-0">
                                <div className="flex items-center justify-between px-2 shrink-0">
                                    <h3 className="text-[11px] font-black uppercase tracking-[0.3em] opacity-40">Domain Totals</h3>
                                    <div className="relative w-64">
                                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 opacity-20" />
                                        <input
                                            type="text"
                                            placeholder="Audit domain..."
                                            value={searchQuery}
                                ...
                                <div className="flex-1 overflow-y-auto pr-4 space-y-3 custom-scrollbar" data-lenis-prevent>
                                    {tabTotals.length === 0 ? (
                                        <div className="h-full flex flex-col items-center justify-center opacity-10 gap-4">
                                            ...
                                            <p className="text-sm font-black uppercase tracking-[0.5em]">No Signals</p>
                                        </div>
                                    ) : (
                                        tabTotals.map(([domain, data], idx) => (
                                            <motion.div
                                                key={domain}
                                                layout
                            ...
                            <div className="w-full md:w-96 flex flex-col gap-6 min-h-0">
                                <h3 className="text-[11px] font-black uppercase tracking-[0.3em] opacity-40 px-2 shrink-0">Recent Activity</h3>
                                <div className="flex-1 overflow-y-auto pr-4 space-y-4 custom-scrollbar" data-lenis-prevent>
                                    {recentTabs.slice(0, 20).map((tab, idx) => (
